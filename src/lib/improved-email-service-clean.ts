...
class ImprovedEmailService {
  ...
  private async verifyConnections() {
    ...
    if (this.backupTransporter) {
      try {
        ...
        logger.error('Backup SMTP connection failed:', { error });
  ...
  private checkRateLimit(email: string): { 
    ...
    // Global rate limiting: max 1 email per 2 seconds (more conservative)
    if (now - this.lastSendTime < 2000) {
      return { 
        allowed: false, 
        message: 'Please wait a moment before sending another email',
        waitTime: Math.ceil((2000 - (now - this.lastSendTime)) / 1000)
      };
    }
    ...
    if (userLimit.cooldownUntil && now >= userLimit.cooldownUntil) {
      delete userLimit.cooldownUntil;
      userLimit.count = 0;
    ...
    // Progressive rate limiting based on count
    let maxEmails = 3; // Start conservative
    let minInterval = thirtySeconds;
  ...
  private async sendWithTransporter(
    ...
    try {
      ...
      logger.info(`Sending email via ${transporterName}`, { to: options.to, subject: options.subject });
      logger.info(`SMTP Config`, { from: this.config.from, to: options.to });
      
      const result = await transporter.sendMail(mailOptions);
      
      logger.info(`Email sent successfully via ${transporterName}`, { messageId: result.messageId });
      logger.debug(`Full email result`, { result: JSON.stringify(result, null, 2) });
      
      return { success: true, messageId: result.messageId };
    } catch (error) {
      logger.error(`Failed to send email via ${transporterName}`, { error });
  ...
  async sendEmail(options: EmailOptions): Promise<EmailResult> {
    try {
      ...
      if (this.backupTransporter && !this.isBackupServiceDown) {
        logger.warn('Primary SMTP failed, trying backup service...');
