...
-- NOTE: This assumes user_role type already exists in your database
...
-- =====================================================
-- 3. Fix has_permission function (simplified to use TEXT)
-- =====================================================

DROP FUNCTION IF EXISTS public.has_permission(UUID, user_role);

CREATE OR REPLACE FUNCTION public.has_permission(user_uuid UUID, required_role TEXT)
RETURNS BOOLEAN
SET search_path = ''
AS $$
DECLARE
...
BEGIN
    SELECT role::TEXT INTO user_role_val FROM public.profiles WHERE id = user_uuid;
